...
export default function WorkItem({ data }) {
  return (
    <>
      {data.map((item, index) => {
        return (
          <div id={'project-' + item.id} className="basis-1/3 flex-1 shadow-lg rounded-xl p-10 dark:bg-slate-200" key={index}>
            <h3 className="font-bold text-center text-slate-900">{item.title}</h3>
            <div className="group relative w-52 h-52 mx-auto my-4 md:w-56 md:h-56 lg:w-60 lg:h-60">
              <Link href={item.projectLink}>
                <Image src={item.thumbnail} alt='project-thumbnail' layout="fill" objectFit="cover" />
            ...
            <ul className="text-center text-slate-700">{item.skills.map((skill, num) => <li key={num}>{skill}</li>)}</ul>
          </div>
        )
      })}
    </>
